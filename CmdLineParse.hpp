...
{
  public:
  CmdLineParse() { InitTables(); };
  ~CmdLineParse() {};
  CmdLineParse(int argc, const char * argv[]);

  int Flags(string QString);
  int Vars(string QString);

  private:
  void InitTables(void);

  map<string, int> CmdFlags;
  map<string, int> CmdVars;
